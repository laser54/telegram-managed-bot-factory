...
| Terminal-only first run | Factory has no Desktop launcher or unconfigured MCP bootstrap catalog. The operator runs one interactive command on the Linux worker host; an unconfigured or unhealthy MCP process fails closed with a safe terminal-only repair instruction. |
...
| `systemd --user` with verified lingering is mandatory | The product requires a durable worker. Onboarding checks the user manager and enables/verifies lingering before asking for a credential, then installs the unit and verifies it is active, enabled, and reporting a fresh heartbeat; tmux/background-process fallback is not presented as successful installation. |
